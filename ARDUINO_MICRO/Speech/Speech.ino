/*
per la programmazione con il dongle USB
1)Tenere premuto il reset
2) compilare
3) quando dice uploading. rilasciare
*/
/*
...
#define SPO_ALD   2	//was 24 // cavetto blu =Wr
#define SPO_LRQ   3	//was 23 // è il Busy, in input
#define SPO_RESET 4	//was 22 // needed if we drive reset from the arduino; not otherwise
...
#define PIN_BUSY 12	//Segnale di busy verso HOST
#define BUSY_ON digitalWrite(PIN_BUSY, 1);  
#define BUSY_OFF digitalWrite(PIN_BUSY, 0);
#define ON 1
#define OFF 0
#include <SP0256-AL2\SP0256-AL2micro.h>
...
void setBusy(bool status) {
	if (status == ON)
	{
		...
		digitalWriteFast(13, 1);// ACCENDO IL LED
	}
	else
	{
		// invia * solo alla prima chiamata dallo stato Busy ON
		if digitalReadFast(13) {
			Serial.print("*");

		}
		digitalWriteFast(13, 0);//SPENGO IL LED
...
void setup() {
	...
	Serial.println("SP0256-AL2 Terminal; send 't' for test.");
	...
	#pragma region Setup SP0256-AL2
	...
	//  give the SPO time to get initialized
	delay(1000);
...
void loop() {
	// attende dati su seriale
	//while(!Serial.available()>0){}

	// mette i dati nella pila
	while (Serial.available()>0)
	{
		s = Serial.readString();
